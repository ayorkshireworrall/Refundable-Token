 ...
 * behavior.
 * Adapted to work with solidity ^0.8.0 from a deprecated openzeppelin contract available on an
 * old branch: https://github.com/OpenZeppelin/openzeppelin-contracts/tree/release-v2.5.0/contracts/crowdsale
 */
abstract contract Crowdsale is Context, ReentrancyGuard {
    ...
    event TokensToCreate(uint256 amount);

    event GeneralLog(uint amout);
    ...
    function buyTokens(address beneficiary) public nonReentrant payable {
        ...
        emit GeneralLog(weiAmount);

        // calculate token amount to be created
        uint256 tokens = _getTokenAmount(weiAmount);
        emit GeneralLog(tokens);
